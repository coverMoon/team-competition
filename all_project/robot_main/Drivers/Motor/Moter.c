...
static uint16_t target_pos = 0;                          // 底盘目标位置（rad，供监测任务读取）
...
void chassis_init(void)
{
    ...
    cybergear_motor_stop(mi_motor);                          // 停止电机，确保初始化安全
    cybergear_motor_set_mode(mi_motor, MOTOR_CONTROL_MODE_POSITION); // 设置为位置控制模式
    HAL_Delay(2);                                            // 短暂延迟，等待模式切换生效
    cybergear_motor_set_current_limit(mi_motor, 15.0f);      // 设置最大电流限制为15A
    HAL_Delay(2);                                            // 等待电流参数写入生效
    cybergear_motor_enable(mi_motor);                        // 使能电机，允许接收控制指令
    HAL_Delay(2);                                            // 等待电机启动稳定
//    cybergear_motor_request_parameter(mi_motor, PARAM_MECH_POS); // 主动请求机械位置参数
//    HAL_Delay(2);                                            // 等待位置数据反馈
}
...
void dji_init(void)
{
    dji_motor_zhuzhou = dji_motor_get_instance(2);  // 获取主轴电机实例（索引0）
    dji_motor_dabi = dji_motor_get_instance(1);     // 获取大臂电机实例（索引1）
    dji_motor_xiaobi = dji_motor_get_instance(0);   // 获取小臂电机实例（索引4）
}
...
{
    int32_t angle = dji_degree2encoder(height * 30);  // 高度→编码器角度换算（待调试确认）
    dji_motor_set_location(dji_motor_zhuzhou, angle);  // 发送目标位置指令
}
...
{
    dji_motor_set_location(dji_motor_xiaobi, dji_degree2encoder(angle));  // 发送小臂目标角度指令
}
...
{
    int32_t angle = dji_degree2encoder(length );  // 长度→编码器角度换算（待调试确认）
...
void xipan_control(int open)
{
    if(open == 1)
    {
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, GPIO_PIN_SET);  // 引脚置高，吸盘吸合
    }
    else
    {
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, GPIO_PIN_RESET); // 引脚置低，吸盘释放
    }
}
